...
5. **A missing on-demand plugin offers installation.** When `entire-<name>` is absent for a name in `onDemandInstallPluginNames` (`graph`, `investigate`), `entire <name> <args>` resolves that name in the configured plugin index and asks `Install the entire-<name> plugin from <repo url>?` with Yes selected by default. The repository is named because this prompt is the only human checkpoint on the path — an index-listed install never prompts inside `runRemoteInstall`, since the catalog is the trust decision. A name the index does not carry is reported instead of offered, rather than prompting and then failing on a request that could never be honored. Accepting installs the plugin through the normal managed installer, then executes the installed binary with all remaining arguments unchanged. This also works for the bare command and its `--help`. Installation output goes to stderr, and the run that follows is announced as `Running entire-<name>` — the binary's name only, never the arguments, which are the user's own command line and could carry a token, a newline or a terminal escape into stderr and anything capturing it. Declining or a failed installation exits nonzero without running the command; a cancelled one terminates by signal, so a single Ctrl-C escapes an enclosing shell loop. Non-interactive sessions receive an `entire plugin install <name>` hint instead of a prompt. Other missing plugin names still fall through to Cobra.

   The set is deliberately narrow, and membership is not a reward for being official — `run` and `upgrade` are both index-listed and both absent from it. The offer is a prompt defaulting to Yes that ends in a downloaded binary linked onto `$PATH`, so it belongs only to names this CLI previously answered itself, where a user typing the command has every reason to expect it to exist. `graph` is the on-demand semantic index; `investigate` was a built-in until it moved to the entire-investigate plugin, and without the offer an established command would answer with Cobra's "unknown command".

   A name in the set must be resolvable through the plugin index, because the install path looks it up there and nowhere else. An unlisted name converts a clean fall-through into a prompt that can only fail.
